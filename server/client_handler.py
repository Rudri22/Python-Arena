...
def _handle_invitation_message(
    ...
    if action == "send":
        # For "send", the sender must match `from_user`.
        ...
        send_message(
            client_socket,
            make_invitation_message(from_user=from_user, to_user=to_user, action="sent"),
        )
        ...
        if target_socket is not None:
            send_message(
                target_socket,
                make_invitation_message(from_user=from_user, to_user=to_user, action="received"),
            )
        return
...
def handle_incoming_message(
    ...
    if message_type == MessageType.USERNAME.value:
        ...
        # Confirm username acceptance to this client.
