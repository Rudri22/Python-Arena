...
class ServerState:
    ...
    # Compatibility helper kept for existing caller style.
    def set_username(self, client_id: str, username: str) -> list[str]:
        """Set username or raise ValueError; return current online list."""

        ok, reason = self.set_client_username(client_id, username)
        if not ok:
    ...
    def get_username_for_client(self, client_id: str) -> str | None:
        """Resolve username for one client id."""

        return self.get_client_username(client_id)
    ...
    def get_client_sockets_snapshot(self) -> list[socket.socket]:
        ...
        return self.get_client_sockets()
    ...
    def get_socket_by_username(self, username: str) -> socket.socket | None:
        """Find connected socket by username."""

        return self.get_socket_for_username(username)
