...
TERRAIN_ASSETS_DIR = Path(__file__).resolve().parents[1] / "assets" / "terrain"
...
class PygameArenaWindow:
    ...
    def __init__(
        ...
        self.tile_sand: pygame.Surface | None = None
        self.tile_sand_slope: pygame.Surface | None = None
        self.tile_grass: pygame.Surface | None = None
        self.tile_grass_slope: pygame.Surface | None = None
        self.tile_stone: pygame.Surface | None = None
        self.tile_water: pygame.Surface | None = None
        self.tile_dirt: pygame.Surface | None = None
        self.arena_board_surface: pygame.Surface | None = None
        self._load_terrain_tiles()
        self._build_arena_board_surface()
    ...
    def _load_terrain_tile(self, filename: str) -> pygame.Surface | None:
        """Load one terrain tile from assets and resize to the game cell size."""

        path = TERRAIN_ASSETS_DIR / filename
        if not path.exists():
            return None
        try:
            raw = pygame.image.load(str(path)).convert_alpha()
            # Keep crisp pixels to avoid alpha seams between adjacent cells.
            return pygame.transform.scale(raw, (CELL_SIZE, CELL_SIZE))
        except pygame.error:
            return None

    def _load_terrain_tiles(self) -> None:
        """Load clean raw sand/grass tiles used for the arena floor."""

        sand = self._load_terrain_tile("tileSand.png")
        sand_slope = self._load_terrain_tile("tileSand_slope.png")
        grass = self._load_terrain_tile("tileGrass.png")
        grass_slope = self._load_terrain_tile("tileGrass_slope.png")
        stone = self._load_terrain_tile("tileStone.png")
        water = self._load_terrain_tile("tileWater_1.png")
        dirt = self._load_terrain_tile("tileDirt.png")

        if not sand or not sand_slope or not grass or not grass_slope:
            self.tile_sand = None
            self.tile_sand_slope = None
            self.tile_grass = None
            self.tile_grass_slope = None
            return

        self.tile_sand = sand
        self.tile_sand_slope = sand_slope
        self.tile_grass = grass
        self.tile_grass_slope = grass_slope
        self.tile_stone = stone
        self.tile_water = water
        self.tile_dirt = dirt

    def _build_arena_board_surface(self) -> None:
        """Pre-render colorful map-like arena ground (grass/sand/stone/water patches)."""

        if not self.tile_sand or not self.tile_sand_slope or not self.tile_grass or not self.tile_grass_slope:
            self.arena_board_surface = None
            return

        board = pygame.Surface((self.board_pixel_width, self.board_pixel_height), pygame.SRCALPHA)

        # Bigger map blocks (fewer rows/cols visible) to mimic the sample style.
        chunk = 2
        tile_w = CELL_SIZE * chunk
        tile_h = CELL_SIZE * chunk
        map_cols = BOARD_COLS // chunk
        map_rows = BOARD_ROWS // chunk

        # Build deterministic terrain map.
        terrain: list[list[str]] = []
        for r in range(map_rows):
            row: list[str] = []
            for c in range(map_cols):
                base = "grass" if c < (map_cols // 2) else "sand"
                v = (r * 37 + c * 53 + r * c * 7) % 23
                if v in {0, 11}:
                    base = "stone"
                if v in {5, 17}:
                    base = "dirt"
        ...
        # Add map-like water lanes.
        water_row = map_rows // 2
        for c in range(1, map_cols - 1):
            terrain[water_row][c] = "water"
        water_col = (map_cols * 3) // 5
        for r in range(2, map_rows - 2):
            terrain[r][water_col] = "water"

        # Keep bottom strip grounded.
        for c in range(map_cols):
            if terrain[map_rows - 1][c] == "water":
                terrain[map_rows - 1][c] = "sand" if c >= (map_cols // 2) else "grass"

        for r in range(map_rows):
            for c in range(map_cols):
                kind = terrain[r][c]
                if kind == "water":
                    src = self.tile_water or self.tile_grass
                elif kind == "stone":
                    src = self.tile_stone or self.tile_grass
                elif kind == "dirt":
                    src = self.tile_dirt or self.tile_sand
                elif kind == "sand":
                    src = self.tile_sand
                else:
                    src = self.tile_grass

                draw_tile = pygame.transform.scale(src, (tile_w, tile_h))
                board.blit(draw_tile, (c * tile_w, r * tile_h))

        # Gentle color correction toward the sample palette.
        warm = pygame.Surface((self.board_pixel_width, self.board_pixel_height), pygame.SRCALPHA)
        warm.fill((236, 164, 78, 36))
        board.blit(warm, (0, 0), special_flags=pygame.BLEND_RGBA_ADD)
        cool = pygame.Surface((self.board_pixel_width, self.board_pixel_height), pygame.SRCALPHA)
        cool.fill((46, 188, 108, 28))
        board.blit(cool, (self.board_pixel_width // 2, 0), special_flags=pygame.BLEND_RGBA_ADD)

        self.arena_board_surface = board
    ...
    def _draw_frame(self) -> None:
        """Render tiles-only arena view (no HUD/text/borders/entities)."""
    ...
    def _draw_board(self) -> None:
        """Draw arena board with biome terrain background (no grid overlay)."""
        ...
        pygame.draw.rect(self.screen, BOARD_BG_COLOR, board_rect)
        if self.arena_board_surface is not None:
            self.screen.blit(self.arena_board_surface, (self.board_origin_x, self.board_origin_y))
        # Intentionally no border in tiles-only mode.

        # Intentional: no visible cell grid; keep terrain as continuous connected blocks.
